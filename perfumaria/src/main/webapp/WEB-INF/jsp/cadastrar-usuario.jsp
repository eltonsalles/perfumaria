...
<div class="col-md-10 content">
    <h2><c:out value="${sessionScope.usuario.id > 0 ? 'Alterar' : 'Cadastrar'}"></c:out> Usu?rio</h2>
    ...
    <form action="sistema?controller=Usuario&action=<c:out value="${sessionScope.usuario.id > 0 ? 'editar' : 'novo'}"></c:out>" method="post" class="col-md-4">
            ...
            <div class="form-group col-md-12 <c:if test="${errorValidation.funcionario eq true}">has-error</c:if>">
                <label class="control-label" for="funcionario">C?digo do Funcion?rio</label>
                <input type="text" class="form-control" id="funcionario" placeholder="C?digo do Funcion?rio" name="funcionario" maxlength="10" required pattern="^\d+$" value="<c:out value="${sessionScope.usuario.funcionario.id}"></c:out>" <c:if test="${sessionScope.usuario.id > 0}">readonly</c:if>>
            </div>
            ...
            <div class="form-group col-md-12 <c:if test="${errorValidation.nivelUsuario eq true}">has-error</c:if>">
                <label class="control-label" for="nivel-usuario">N?vel de Usu?rio</label>
                ...
                <c:set var="nivelUsuario" value="${sessionScope.usuario.nivelUsuario.id}"></c:set>
                    ...
                    <option value="7" <c:if test="${fn:containsIgnoreCase(nivelUsuario, '7')}">selected</c:if> >Suporte T?cnico</option>
