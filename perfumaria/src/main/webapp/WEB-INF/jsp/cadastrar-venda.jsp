    ...
    <form id="form-venda" action="sistema?controller=Venda&action=novo" method="post">
        ...
        <div class="form-group col-md-3 <c:if test="${errorValidation.total eq true}">has-error</c:if>">
            ...
            <div class="input-group">
                ...
                <input type="text" class="form-control" id="total" placeholder="x.xxx,xx" name="total" maxLength="12" required readonly="readonly" pattern="(\d{1,3}\.)?\d{1,3},\d{2}$" value="<c:out value="${not empty sessionScope.data.total[0] ? sessionScope.data.total[0] : '0,00'}"></c:out>">
            </div>
        </div>
